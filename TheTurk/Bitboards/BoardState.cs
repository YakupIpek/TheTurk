...
public class BoardState
{
    ...
    public void PlayNullMove(BoardState from)
    {
        ...
        EnPassant = 0;
        ZobristKey = from.ZobristKey;

        //Update Zobrist-Hash
        ZobristKey ^= Zobrist.SideToMove;
        if (from.EnPassant > 0)
            ZobristKey ^= Zobrist.Flags(LSB(from.EnPassant));
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public bool PlayWithoutHashAndEval(BoardState from, Move move)
    {
        if (from.SideToMove == Color.White)
        {
            PlayWhite(from, ref move);
    ...
    public bool IsAttackedByBlack(int square)
    {
        ...
        var x = 0xFEFEFEFEFEFEFEFEUL;
        var y = 0x7F7F7F7F7F7F7F7FUL;
    ...
    {
        //Side to move
        ZobristKey = SideToMove == Color.Black ? Zobrist.SideToMove : 0;

        //Pieces
        for (ulong bits = White | Black; bits != 0; bits = ClearLSB(bits))
        {
            int square = LSB(bits);
            ZobristKey ^= Zobrist.PieceSquare(GetPiece(square), square);
        }

        //En passent & Castling
        for (ulong bits = CastleFlags | EnPassant; bits != 0; bits = ClearLSB(bits))
            ZobristKey ^= Zobrist.Flags(LSB(bits));
    }
    ...
    public void UpdateHash(BoardState from, ref Move move)
    {
        ...
        ZobristKey ^= Zobrist.SideToMove;
        ZobristKey ^= Zobrist.PieceSquare(move.MovingPiece(), move.FromSquare);
        ZobristKey ^= Zobrist.PieceSquare(move.CapturedPiece(), move.ToSquare);
        ZobristKey ^= Zobrist.PieceSquare(move.NewPiece(), move.ToSquare);

        switch (move.Flags)
        {
            case Piece.EnPassant | Piece.BlackPawn:
                ZobristKey ^= Zobrist.PieceSquare(Piece.WhitePawn, move.ToSquare + 8);
                break;
            case Piece.EnPassant | Piece.WhitePawn:
                ZobristKey ^= Zobrist.PieceSquare(Piece.BlackPawn, move.ToSquare - 8);
                break;
            case Piece.CastleShort | Piece.Black:
                ZobristKey ^= Zobrist.PieceSquare(Piece.BlackRook, 63);
                ZobristKey ^= Zobrist.PieceSquare(Piece.BlackRook, 61);
                break;
            case Piece.CastleLong | Piece.Black:
                ZobristKey ^= Zobrist.PieceSquare(Piece.BlackRook, 56);
                ZobristKey ^= Zobrist.PieceSquare(Piece.BlackRook, 59);
                break;
            case Piece.CastleShort | Piece.White:
                ZobristKey ^= Zobrist.PieceSquare(Piece.WhiteRook, 7);
                ZobristKey ^= Zobrist.PieceSquare(Piece.WhiteRook, 5);
                break;
            case Piece.CastleLong | Piece.White:
                ZobristKey ^= Zobrist.PieceSquare(Piece.WhiteRook, 0);
                ZobristKey ^= Zobrist.PieceSquare(Piece.WhiteRook, 3);
                break;
        }
